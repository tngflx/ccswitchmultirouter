...
### Codex 报 `502 Bad Gateway: http://127.0.0.1:15721/v1/responses`

**先区分两种边界**：
- **请求没有到达 CC Switch**：Codex Desktop 自己走了系统代理、规则代理或 VPN，代理把 `127.0.0.1:15721` 当成外部目标处理并返回 502。这时 `~/.cc-switch/logs/codex-router.log` 里通常看不到同一时间的新请求。
- **请求已进入 CC Switch，但出站失败**：日志里能看到 `route_resolved`、`request_prepared`、`upstream_send_error`，说明本地 MultiRouter 已命中，失败在 CC Switch 到真实上游（如 ChatGPT、DeepSeek、Qwen）的网络链路。

**排查步骤**：
1. 在 Codex MultiRouter 页面运行「Debug / 一键诊断」，查看「出站代理 / VPN 环境」和「近期路由错误」。
2. 如果没有近期 router 日志，让系统代理或 VPN 明确绕过 `localhost`、`127.0.0.1` 和 `::1`；部分规则代理需要改为全局代理或开启 TUN 模式才能让 Codex 与 CC Switch 进程走同一条网络路径。
3. 如果日志里有 `upstream_send_error`，展开同一 trace 的 `upstream_url`、`effective_provider` 和 `responses_to_chat`，按真实上游排查网络、额度、账号或供应商服务状态。
4. 如果开启全局代理后恢复，而规则代理失败，优先把 CC Switch 进程、Codex Desktop 进程和真实上游域名放进同一代理策略，不要只配置浏览器代理。
