...
### codex_oauth Responses SSE 中途断流自动重连

Codex OAuth 上游会在 HTTP 200 之后、SSE 正文未走完时偶发掐断连接；此前 ccsm
直接把它升级成 `502 error decoding response body`。本版在代理侧补齐官方 Codex
CLI 已有的 `stream_max_retries=5` 语义：仅当下游还没收到实质内容时重连，最多 5
次，退避 200/400/800/1600/3200ms；上游语义错误和已转发内容不重试。同时为
codex_oauth 开启 thinking 时注入 `reasoning.summary:"auto"`，并在思考静默期每
10s 发一次 `ping`，避免 GPT-5.x 长思考被空闲计时器误杀。
