...
- **Codex Chat Prompt-Cache Routing Covers More Compatible Gateways and Explains Its Limits**: Auto mode now sends `prompt_cache_key` to Azure OpenAI resource endpoints and OpenRouter in addition to OpenAI and Kimi Coding. The provider form now explains that this key improves cache affinity for repeated stable prefixes rather than guaranteeing a cache hit, names the automatically supported gateways, warns that unknown strict-schema gateways can return HTTP 400, and points to `usage.cached_tokens` as the evidence of an actual hit. Gateways with different cache protocols remain off unless explicitly enabled.
