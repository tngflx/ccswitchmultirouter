...
- 正式发布：`v3.16.5-24`（commit `03bf30a4`，分支 `fix/3.16.5-22-gpt-live`）与 `v3.19.0-3`（commit `da29b49a`，分支 `bigstrongsun/ccsm-agent-mesh`）已由 GitHub Release workflow 构建并发布，`isPrerelease=false`，均含 19 个跨平台资产和 `latest.json`。Release workflow 会读取 `docs/release-notes/v<tag>-zh.md` 作为正文，因此新版本必须先补该文件再 push tag。
