...
# 2026-08-07 Codex Desktop 国际化占位符原样显示根因（仅诊断，尚未修复）
...
- 真正修复应收紧注入边界：只有对象本身具有明确模型门特征时才允许写 `defaultModel/useHiddenModels`；模型列表 patch 应只处理已验证的 app-server/model-list/Statsig 结果。禁止再对整棵 React Fiber 做通用对象写入，并应增加浏览器级回归：注入前后 `intl.formats/defaultFormats` 无新增键，`formatMessage` 可完成 scalar/rich-text 插值，DOM 不含未解析 ICU/tag 占位符。此轮按用户要求只分析，未修改实现。
