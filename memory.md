...
- `#10` 的 portable 前提同时可能指 CCSwitchMulti 和 Codex Desktop 都来自 zip。不要恢复前端“手动选择 Codex.exe”主流程，也不要把路径写进 `ccswitch.codexDesktopExecutablePath`；正确边界是后端在用户点“解锁模型菜单”且发现正在运行的大写 `Codex.exe` 时，把该 Desktop shell 路径记到 `.cc-switch/codex-desktop-executable.json`，用于后续复用已校验路径或覆盖非标准安装发现失败。
