...
- 实施位于独立分支 `bigstrongsun/codex-terminal-semantics`；未安装、重启、修改 live Provider/Router 配置或推送。Sub-Agent V2 只消费相同 Responses 事件，不是根因；协议选择决定走 Chat 还是 Responses 适配器，但两条路径必须遵守同一完成契约。
