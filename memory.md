...
- 发布前已用 merge commit `3044215a` 合入公开 `v3.19.1-20@8de0b422` 的 OAuth 过期竞态、自启动漂移、小窗口 AppSwitcher 和 Responses 断流修复，四个版本文件统一保留 `3.19.1-21`。合并后定向回归为 `useManagedAuth + AppSwitcher 4/4`、`streaming_retry 22/22`、`auto_launch 4/4`；发布仍必须在最终 release-note/memory 提交上重跑完整 Rust、Vitest、Pester、check/fmt/typecheck/Prettier/diff 门禁，再推 annotated tag。
