...
- 正式本地发布流水线使用项目保存的 Tauri updater 私钥完成 `3.19.2-9` NSIS 构建、签名与导出；文档 checkpoint `515ccb05` 的导出验证为 12,360,968 bytes、SHA-256 `fde0e1d609414d065c3978e6f437460924c7d4d7e740a355b1389a7f0545fc26`，`latest.json` signature 与相邻 `.sig` 精确一致，metadata commit 为 `515ccb054f422e82361333926a833495af514a1a`。NSIS 包含构建时间信息，同一源码重建的字节数/哈希可变化；权威路径固定为 `C:\Users\sunda\Documents\LLMservice\最新版ccswitchmulti\windows\installer\CCSwitchMulti_3.19.2-9_x64-setup.exe`，每次交付必须对当前文件重算哈希并核对 metadata/signature，不能沿用 memory 中的历史哈希。裸 `pnpm build` 会尝试本机 WiX MSI 并在 `light.exe` 失败；裸 `pnpm tauri build --bundles nsis` 虽生成 bundle，但因没有注入私钥在 updater 签名门禁退出 1，不能用它覆盖正式导出产物。
...
- 根修实现：普通 `targetProviderId` route 现在保存 `upstream.apiFormatSource=provider`，`syncCodexMultiRouterPlanWithProviders` 每次 Provider 保存同步目标协议，覆盖 Chat→Responses 与 Responses→Chat；历史无来源标记的普通引用 route 默认迁移为 Provider 继承，因此能自动修复现场 Qwen 脏数据。路由编辑器手工选择协议会标记 `route_override`；DeepSeek Flash/Pro 历史拆分中与 Provider 不同的协议也迁移为显式 override，避免同步抹平有意的混合协议窗口。
- TDD/验证：两个双向同步用例在旧实现上按预期 RED，修复后同步/向导/工作台目标测试 100/100、`pnpm typecheck`、变更文件 Prettier 和 `git diff --check` 通过。完整前端测试 1130/1131，唯一失败为既有 `CodexFormFields` `scrollIntoView` 时序断言；单文件复跑 28/28 通过，未复现为本次协议同步回归。
