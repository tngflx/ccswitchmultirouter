...
- 安装态 catalog 真实值为 Qwen3.8 `supports_image_detail_original=false` 且 `supportsImageDetailOriginal=false`，官方 `gpt-5.4` 仍为 true。最小同形 canary 将 `function_call(view_image) -> function_call_output(input_image, detail=original) -> 后续文本轮` 发送到安装代理，trace `aabed260-f014-4d4b-9e98-1ea3fcf0d967` 走 `/responses -> /chat/completions`、HTTP 200，并收到 `response.completed` 与 `CCSM_QWEN38_ORIGINAL_REPLAY_OK`。
