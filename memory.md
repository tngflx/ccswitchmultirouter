...
- 首次本地替换事务正确检测到安装后 EXE 哈希不匹配并完成自动回滚：旧安装哈希 `34618BEC...E403`、新运行态实际哈希 `FF4255A1...3644`、当时误用的构建目录 EXE 哈希 `53CC5565...CF22`，`transaction-result.json` 为 `RolledBack` 且 `RollbackError=null`，回滚后 PID 64616、15721/health 200、旧哈希恢复。Tauri/NSIS 会为 bundle 类型处理打包输入，不能把构建目录根 EXE 哈希直接当作安装后哈希；本次新运行态真实哈希只能采用与 installer SHA-256 `3B203045...5C27` 绑定的事务 readback `FF4255A1...3644`。
