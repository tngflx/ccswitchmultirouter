...
- 2026-06-28 普通 Codex provider 表单“获取模型”路径加入保守拆分提示：当 `/models` 同时返回 GPT-like（如 `gpt-*`、OpenAI namespace 下的 gpt、o 系列）和非 GPT-like 模型，且当前表单还没有用户手写 route 时，`CodexFormFields` 只弹出“检测到混合协议模型”确认框，不会静默写入 routing。用户确认后才会打开本地接管、把顶层 apiFormat 设为 `openai_responses`，并生成两条内联 `codexRouting` route：`<providerName>-responses` 走 `openai_responses` 且不写 `modelMap`，`<providerName>-chat` 走 `openai_chat` 并写 identity `modelMap`；用户取消时只保留已获取的模型列表，routing、接管开关和 apiFormat 都不变。
