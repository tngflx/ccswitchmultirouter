...
pub(crate) fn chat_media_part_from_tool_part(part: &Value, scope: ToolMediaScope) -> Option<Value> {
    ...
    let chat_part = match kind {
        ...
        ToolMediaKind::Audio => chat_audio_from_input_audio(part).map(|input_audio| {
            json!({
                "type": "input_audio",
...
/// Map both current Chat-style `input_audio` objects and Responses-compatible
/// `audio_url` data URLs to the Chat audio payload.
pub(crate) fn chat_audio_from_input_audio(part: &Value) -> Option<Value> {
    if let Some(input_audio) = part.get("input_audio").filter(|value| value.is_object()) {
        return Some(input_audio.clone());
    }

    let value = part.get("audio_url").and_then(Value::as_str)?;
...
fn strip_media_from_tool_value_at_depth(
    ...
    match value {
        ...
        Value::Object(_) => {
            ...
            if scope == ToolMediaScope::AllSupported {
                if let Some(replacement) = sensitive_tool_output_replacement(value) {
                    *value = replacement;
                    return 1;
                }
            }
...
fn tool_media_kind(part: &Value) -> Option<ToolMediaKind> {
    ...
    match part_type {
        ...
        Some("input_audio") if chat_audio_from_input_audio(part).is_some() => {
...
fn sensitive_tool_output_replacement(part: &Value) -> Option<Value> {
    let object = part.as_object()?;
    let part_type = object.get("type").and_then(Value::as_str)?;

    let marker = if part_type == "encrypted_content" {
        Some("[cc-switch: encrypted content omitted]")
    } else if part_type == "input_audio"
        && (object.contains_key("audio_url") || object.contains_key("input_audio"))
    {
        Some("[cc-switch: unsupported structured audio omitted]")
    } else if part_type == "input_file"
        && (object.contains_key("file_url")
            || object.contains_key("file_data")
            || object.contains_key("file_id"))
    {
        Some("[cc-switch: unsupported structured file omitted]")
    } else if !matches!(part_type, "image" | "video" | "text" | "input_text")
        && (object.contains_key("encrypted_content")
            || object.contains_key("data")
            || object.contains_key("file_data"))
    {
        Some("[cc-switch: unsupported structured tool output omitted]")
    } else {
        None
    }?;

    Some(json!({"type": "text", "text": marker}))
}
