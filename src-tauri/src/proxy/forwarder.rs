...
pub struct RequestForwarder {
    ...
    /// Request-health correlation id, generated by RequestContext before forwarding.
    request_health_trace: String,
...
impl RequestForwarder {
    ...
    > {
        ...
        let codex_trace_id =
            matches!(app_type, AppType::Codex).then(|| self.request_health_trace.clone());
        ...
        if matches!(app_type, AppType::Codex) {
            let review_dispatch_scope = format!("{}|{}|{}", provider.id, method.as_str(), url);
            let review_result = super::request_health::review_before_upstream(
                &request_health_config,
                codex_trace_id.as_deref().unwrap_or_default(),
                &self.session_id,
                self.session_client_provided,
                outbound_model
                    .as_deref()
                    .unwrap_or(request_model_for_log.as_str()),
                &body_bytes,
                &review_dispatch_scope,
                codex_request_metadata.request_kind == "compaction",
            )
            .await;
                ...
                if let Some(trace_id) = codex_trace_id.as_deref() {
                    ...
                    super::codex_router_log::append_event(
                        ...
                        &[
                            ...
                            (
                                "wire_body_hash",
                                super::request_health::wire_body_hash(&body_bytes),
                            ),
                            ("reason", error.clone()),
                        ],
                    );
                }
                return Err(ProxyError::RequestHealthBlocked(error));
            }
            if matches!(
                review_result,
            ...
            if matches!(
                review_result,
                Ok(super::request_health::PreflightReviewOutcome::ContinueOnce)
            ) {
                if let Some(trace_id) = codex_trace_id.as_deref() {
                    super::codex_router_log::append_event(
                        "request_health_review_approved",
                        &[
                            ("trace", trace_id.to_string()),
                            ("session", self.session_id.clone()),
                            ("provider", provider.id.clone()),
        ...
        if let Some(trace_id) = codex_trace_id.as_deref() {
            super::codex_router_log::append_event(
                ...
                &[
                    ...
                    ("client_request_bytes", client_request_bytes.to_string()),
                    (
    ...
    fn categorize_proxy_error(&self, error: &ProxyError, provider: &Provider) -> ErrorCategory {
        ...
        match error {
            ...
            ProxyError::RequestHealthBlocked(_) => ErrorCategory::NonRetryable,
            // 网络和上游错误：都应该尝试下一个供应商
...
mod tests {
    ...
    #[tokio::test]
    async fn hosted_streaming_wiring_never_manufactures_summary_from_raw_reasoning() {
        let db = Database::memory().expect("memory database");
        let provider = verified_qwen_hosted_profile(&db);
        let initial: ChatSseStream = Box::pin(futures::stream::iter(vec![Ok::<_, std::io::Error>(
            Bytes::from_static(
    ...
    fn test_forwarder(
        ...
        RequestForwarder {
            ...
            request_health_trace: "test-request-health-trace".to_string(),
