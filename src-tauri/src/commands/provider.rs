...
pub async fn add_provider(
    ...
    let app_type = AppType::from_str(&app).map_err(|e| e.to_string())?;
    // Retained only for IPC compatibility with older frontends. Generic
    // persistence commands never perform upstream verification; callers must
    // use the dedicated protocol-probe command explicitly.
