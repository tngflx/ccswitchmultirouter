    ...
    let catalog_models = specs
        ...
        .map(|spec| {
            ...
            SubagentCatalogModel {
                ...
                supported_reasoning_efforts: spec
                    .reasoning
                    .as_ref()
                    .filter(|capability| capability.supported)
                    .into_iter()
                    .flat_map(|capability| capability.supported_efforts.iter())
                    .filter_map(|effort| SubagentReasoningEffort::from_catalog_value(effort))
                    .collect(),
                default_reasoning_effort: spec
                    .reasoning
                    .as_ref()
                    .filter(|capability| capability.supported)
                    .and_then(|capability| capability.default_effort.as_deref())
                    .and_then(SubagentReasoningEffort::from_catalog_value),
            }
        })
        .collect();
...
mod tests {
        ...
        assert_eq!(
            ...
            json!({
                ...
                "profiles": [{
                    ...
                    "status": "generated",
                    "warnings": ["Model `neutral-model` does not declare selectable reasoning efforts; the managed role omits model_reasoning_effort instead of pinning an invalid value."]
                }],
                "warnings": ["Model `neutral-model` does not declare selectable reasoning efforts; the managed role omits model_reasoning_effort instead of pinning an invalid value."]
            })
        );
    ...
    fn codex_subagent_v2_real_sync_honors_overrides_and_declared_user_name() {
        ...
        assert!(managed.contains("model_reasoning_effort = \"high\""));
        assert!(
            !stale.exists(),
    ...
    fn codex_subagent_v2_same_retained_settings_survive_v1_cleanup_and_v2_restore() {
        ...
        assert!(first.contains("model_reasoning_effort = \"high\""));
