...
pub(crate) const CC_SWITCH_SUBAGENT_V2_POLICY_BEGIN: &str =
    "[CCSWITCHMULTI_SUBAGENT_V2_POLICY_BEGIN]";
pub(crate) const CC_SWITCH_SUBAGENT_V2_POLICY_END: &str = "[CCSWITCHMULTI_SUBAGENT_V2_POLICY_END]";
...
/// Return only the CCSwitchMulti-owned Sub-Agent V2 policy block.
///
/// `developer_instructions` is a shared field: CCSwitchMulti owns the policy
/// block, while everything outside the markers belongs to the user. Ownership
/// consumers (drift fingerprinting, non-destructive apply) must compare and
/// rewrite the block alone, never the whole string.
pub(crate) fn codex_subagent_v2_policy_block(value: &str) -> Option<String> {
    let begin = value.find(CC_SWITCH_SUBAGENT_V2_POLICY_BEGIN)?;
    let relative_end = value[begin..].find(CC_SWITCH_SUBAGENT_V2_POLICY_END)?;
    Some(value[begin..begin + relative_end + CC_SWITCH_SUBAGENT_V2_POLICY_END.len()].to_string())
}

pub(crate) fn strip_codex_subagent_v2_policy_block(value: &str) -> String {
