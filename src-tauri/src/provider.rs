...
pub struct ProviderMeta {
    ...
    /// - "passthrough": 不修改 reasoning item（默认，兼容严格 OpenAI schema）
    /// - "inject_content": 注入 reasoning_text content（DeepSeek 等需要回传可读推理）
    #[serde(rename = "reasoningContentMode", skip_serializing_if = "Option::is_none")]
