...
unsafe extern "system" fn thread_event_target_callback<T: 'static>(
  ...
  let callback = || match msg {
    ...
    win32wm::WM_ENDSESSION => {
      ...
      if end_session_action(wparam) == EndSessionAction::QuitMessageLoop {
        // Do not emit LoopDestroyed while DispatchMessageW is still inside the
        // Win32 message loop. Pending paint/user messages can otherwise try to
        // transition the runner away from Destroyed and panic. WM_QUIT makes
        // GetMessageW return 0; run_return then emits LoopDestroyed exactly once
        // from its normal terminal path.
        PostQuitMessage(0);
      }
...
enum EndSessionAction {
  ContinueMessageLoop,
  QuitMessageLoop,
}

fn end_session_action(wparam: WPARAM) -> EndSessionAction {
  if wparam.0 == TRUE.0 as usize {
    EndSessionAction::QuitMessageLoop
  } else {
    EndSessionAction::ContinueMessageLoop
  }
}

#[cfg(test)]
mod end_session_tests {
  use super::{end_session_action, EndSessionAction};
  use windows::Win32::Foundation::{TRUE, WPARAM};

  #[test]
  fn confirmed_end_session_quits_the_message_loop() {
    assert_eq!(
      end_session_action(WPARAM(TRUE.0 as usize)),
      EndSessionAction::QuitMessageLoop
    );
  }

  #[test]
  fn cancelled_end_session_keeps_the_message_loop_alive() {
    assert_eq!(
      end_session_action(WPARAM(0)),
      EndSessionAction::ContinueMessageLoop
    );
  }
}
