...
function nicknameError(profiles: Record<string, CodexSubagentV2Profile>) {
...
function strengthError(profiles: Record<string, CodexSubagentV2Profile>) {
...
export function CodexSubagentProfileEditor({
  ...
  useEffect(() => {
    ...
    let ignore = false;
    const entries = Object.entries(draft.profiles);
    Promise.all(
  ...
  async function persist(nextConfig: CodexSubagentV2Config) {
    const nextProvider: Provider = {
      ...provider,
      settingsConfig: settingsWithConfig(provider, nextConfig),
    };
  ...
  async function save() {
    if (!draft) return;
    const localError =
      strengthError(draft.profiles) ?? nicknameError(draft.profiles);
    if (localError) {
    ...
    try {
      await Promise.all(
        Object.values(draft.profiles).map((profile) =>
          codexSubagentV2Api.previewProfile(
            draftSettings,
            profile.model,
            profile,
          ),
  ...
  const profileEntries = Object.entries(draft.profiles);
  const primaryEntry =
    profileEntries.find(
      ([, profile]) => profile.model === "deepseek-v4-flash",
    ) ?? profileEntries[0];
  const primaryPreview = primaryEntry ? previews[primaryEntry[0]] : undefined;
  const allWarnings = Array.from(
    new Set([
      ...(primaryPreview?.warnings ?? []),
      ...(statuses?.warnings ?? []),
      ...(statuses?.profiles.flatMap((profile) => profile.warnings) ?? []),
    ]),
  );
  ...
  return (
    <section className="space-y-4 rounded-lg border border-emerald-200 bg-emerald-50/40 p-4 dark:border-emerald-700/50 dark:bg-emerald-950/15">
          ...
          return (
            ...
            >
              ...
              <OverrideField
                ...
                onChange={(value) => setOverride(profileKey, "roleName", value)}
      ...
      {strengthLimitMessage ? (
        <p className="text-sm text-amber-700">{strengthLimitMessage}</p>
      ) : null}
      ...
      <div className="space-y-2 rounded-lg border bg-background/80 p-4 text-sm">
          ...
          <>
            <div className="grid gap-2 md:grid-cols-2">
              <div>
                ...
                <div>{primaryPreview.requestedRoleName}</div>
              </div>
              <div>
                <span className="font-medium">实际角色名</span>
                <div>{primaryPreview.effectiveRoleName}</div>
              </div>
            </div>
            <p>{primaryPreview.description}</p>
            <p>{primaryPreview.developerInstructions}</p>
            <div className="flex flex-wrap gap-2">
              {primaryPreview.nicknameCandidates.map((nickname) => (
                <span key={nickname}>{nickname}</span>
              ))}
            </div>
            <p>{primaryPreview.modelProvider}</p>
            {primaryPreview.modelReasoningEffort ? (
              <p>{primaryPreview.modelReasoningEffort}</p>
            ) : null}
            {primaryPreview.modelContextWindow !== undefined ? (
              <p>{primaryPreview.modelContextWindow}</p>
            ) : null}
          </>
        ) : null}
        ...
        {statuses ? (
          <div className="space-y-2">
            ...
            {statuses.profiles.map((status, index) => (
              <div
              ...
              >
                <p>
                  {status.profileKey
                    ? `${status.profileKey}：${status.status}`
                    : `生成状态：${status.status}`}
                </p>
                {index === 0 ? <p>生成状态：{status.status}</p> : null}
                {index === 0 && status.providerKind ? (
                  <p>Provider 类型：{status.providerKind}</p>
                ) : null}
                <p>可路由：{status.routable ? "是" : "否"}</p>
                {index === 0 && status.enabled !== undefined ? (
                  <p>已启用：{status.enabled ? "是" : "否"}</p>
                ) : null}
                {status.fieldSources ? (
                  <>
                    ...
                    <p>
                      开发者指令来源：
                      {status.fieldSources.developerInstructions}
                    </p>
                    <p>
                      昵称候选来源：{status.fieldSources.nicknameCandidates}
                    </p>
                    <p>
                      模型推理强度来源：
      ...
      <div>
        ...
        {primaryPreview ? (
          <pre className="mt-2 overflow-x-auto whitespace-pre-wrap rounded-lg border bg-slate-950 p-3 text-xs text-slate-100">
            {primaryPreview.tomlPreview}
          </pre>
        ) : null}
      </div>
