...
describe("Codex MultiRouter workspace route persistence helpers", () => {
  ...
  it("does not refresh or overwrite provider catalogs when routes open", async () => {
    ...
    await waitFor(() =>
      expect(screen.getAllByText("OpenAI Enabled").length).toBeGreaterThan(0),
    );
    expect(fetchCodexOauthModels).not.toHaveBeenCalled();
